...
class TestNormalizeTitle:
    def test_strips_punctuation_and_spaces(self):
        from news_collector import _normalize_title
...
class TestDeduplicate:
    def test_removes_exact_duplicate_titles(self):
        from news_collector import _deduplicate
        items = [
...
class TestFetchDuckDuckGo:
    @patch("news_collector.HAS_DDGS", True)
    @patch("news_collector.DDGS")
    def test_returns_formatted_news(self, MockDDGS):
        from news_collector import _fetch_duckduckgo
    ...
    @patch("news_collector.HAS_DDGS", False)
...
class TestFetchEastmoney:
    ...
    def test_parses_json_response(self, mock_get):
        from news_collector import _fetch_eastmoney
        mock_resp = MagicMock()
...
class TestFetchSina:
    ...
    def test_parses_roll_api_response(self, mock_get):
        from news_collector import _fetch_sina
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.raise_for_status = MagicMock()
...
class TestFetchYicai:
    @patch("news_collector.requests.get")
    def test_parses_json_response(self, mock_get):
        from news_collector import _fetch_yicai
        mock_resp = MagicMock()
        mock_resp.raise_for_status = MagicMock()
        mock_resp.json.return_value = [
            {
...
class TestFetchThePaper:
    ...
    def test_parses_hot_news(self, mock_get):
        from news_collector import _fetch_thepaper
        mock_resp = MagicMock()
        mock_resp.raise_for_status = MagicMock()
        mock_resp.json.return_value = {
            "data": {
...
class TestFetchJiemian:
    @patch("news_collector.HAS_BS4", False)
    def test_returns_empty_without_bs4(self):
        from news_collector import _fetch_jiemian
        assert _fetch_jiemian() == []

    @patch("news_collector.HAS_BS4", True)
    @patch("news_collector.requests.get")
    @patch("news_collector.BeautifulSoup")
...
class TestFetchCaixin:
    @patch("news_collector.HAS_BS4", False)
    def test_returns_empty_without_bs4(self):
        from news_collector import _fetch_caixin
...
class TestFetchGuancha:
    @patch("news_collector.HAS_BS4", False)
    def test_returns_empty_without_bs4(self):
        from news_collector import _fetch_guancha
...
class TestCollectAllNews:
    _PATCHES = [
        "news_collector._fetch_guancha",
        "news_collector._fetch_caixin",
        "news_collector._fetch_jiemian",
        "news_collector._fetch_thepaper",
        "news_collector._fetch_yicai",
        "news_collector._fetch_via_agent",
        "news_collector._fetch_sina",
        "news_collector._fetch_eastmoney",
        "news_collector._fetch_duckduckgo",
    ]
...
class TestExecuteWebSearch:
    ...
    def test_returns_json_string(self, MockDDGS):
        from news_collector import _execute_web_search
        mock_instance = MockDDGS.return_value
        mock_instance.news.return_value = [
...
class TestParseAgentResponse:
    def test_parses_plain_json_array(self):
        from news_collector import _parse_agent_response
        text = json.dumps([
...
class TestFetchViaAgent:
    @patch("news_collector.config")
    def test_returns_empty_when_no_api_key(self, mock_config):
        from news_collector import _fetch_via_agent
    ...
    def test_single_tool_call_then_final(self, MockOpenAI, mock_config):
        ...
        with patch("news_collector._execute_web_search", return_value='[{"title":"搜索结果"}]'):
    ...
    def test_respects_max_tool_calls(self, MockOpenAI, mock_config):
        ...
        from news_collector import _fetch_via_agent, _MAX_AGENT_TOOL_CALLS
        mock_config.LLM_API_KEY = "sk-test"
        mock_config.LLM_BASE_URL = "https://api.example.com"
        mock_config.LLM_MODEL = "gpt-4o"
        ...
        with patch("news_collector._execute_web_search", return_value='[]'):
            result = _fetch_via_agent()
    ...
    @patch("news_collector._fetch_agent_fallback")
    @patch("news_collector.config")
    @patch("news_collector.HAS_DDGS", True)
    @patch("news_collector.OpenAI")
...
class TestFetchAgentFallback:
    ...
    def test_generates_keywords_and_merges(self, MockDDGS):
        from news_collector import _fetch_agent_fallback
...
class TestMarketDataGetNewsFallback:
    def test_uses_news_collector_first(self):
        ...
        with patch("news_collector.collect_all_news") as mock_collect, \
             patch("market_data.ak") as mock_ak:
    ...
    def test_falls_back_to_akshare_on_collector_failure(self):
        ...
        with patch("news_collector.collect_all_news", side_effect=Exception("collector broken")), \
             patch("market_data.ak") as mock_ak:
    ...
    def test_returns_empty_when_all_fail(self):
        ...
        with patch("news_collector.collect_all_news", side_effect=Exception("broken")), \
             patch("market_data.ak") as mock_ak:
