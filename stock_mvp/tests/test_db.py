...
class TestMarketSnapshot:
    def test_upsert_and_get_latest(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if 'config' in sys.modules:
                del sys.modules['config']
            from db import Database, MarketSnapshot
            db = Database(db_path)
...
class TestSectorRecommendation:
    def test_upsert_and_get_latest(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if 'config' in sys.modules:
                del sys.modules['config']
            from db import Database, SectorRecommendation
            db = Database(db_path)
...
class TestStockSignal:
    def test_upsert_and_get_latest(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if 'config' in sys.modules:
                del sys.modules['config']
            from db import Database, StockSignal
            db = Database(db_path)
            signal = StockSignal(
                trade_date="2024-01-15",
                stock_code="600000",
...
class TestBackwardCompatibility:
    def test_followed_stocks_still_work(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if 'config' in sys.modules:
                del sys.modules['config']
            from db import Database, FollowedStock
            db = Database(db_path)
            stock = FollowedStock(
                stock_code="600519",
                stock_name="贵州茅台",
